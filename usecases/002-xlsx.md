...
| 维度 | 分数 | 说明 |
|------|------|------|
| 效果 | 4 | 基本解决问题，复杂任务偶有遗漏或「偷懒」用硬编码 |
| 复现成本 | 3 | 安装 LibreOffice 可获得更好效果 |
| 通用性 | 3 | 内置华尔街投行规范，其他行业需要微调 |
...
这是高盛、摩根士丹利、JP 摩根等华尔街投行的内部培训标准。对于非金融行业用户，这个规范可能需要根据自己团队的习惯进行调整。
...
![Skill 输出效果1](./img/002/skill-output.jpg)

![Skill 输出效果2](./img/002/skill-output-2.jpg)

准确填写，格式美观。但在复杂任务中，第二张表并没有完全遵循「公式驱动」的规范。

### WPS (DeepSeek)

![WPS 输出效果](./img/002/wps-output.jpg)

数据准确，但没有用公式，格式不美观。作为内嵌插件，却无法读取选中的内容，仍需语言描述行列位置。

### Formula Bot

![Formula Bot 效果](./img/002/formula-bot.jpg)

效果优秀，主动用颜色标注增强易读性，与 xlsx skill 同属第一梯队。
